...
GNU_HOST=$GNU_BUILD
BUILDDIR=/mnt/HD_a2/build/lzo
PACKAGENAME=lzo
VERSION=2.10
#SOURCEURL=http://www.oberhumer.com/opensource/${PACKAGENAME}/download/${PACKAGENAME}-${VERSION}.tar.gz
SOURCEURL=http://192.168.1.20:88/MyWeb/COMPILER/finish/${PACKAGENAME}-${VERSION}.tar.gz
HOMEURL=http://www.oberhumer.com/opensource/lzo
...
#done
# Remove share dir if it is empty
if [ -d ${BUILDDIR}/ffp/share ]; then
   [ "$(ls -A ${BUILDDIR}/ffp/share)" ] || rm -rf ${BUILDDIR}/ffp/share
fi
PKGDIR="/mnt/HD_a2/ffp0.7arm/packages" makepkg ${PACKAGENAME} ${VERSION} 1
cp -a ${PKGDIR}/${PACKAGENAME}-${VERSION}-arm-1.txz ${PKGDIRBACKUP}
cp -a ${PKGDIR}/${PACKAGENAME}-${VERSION}-arm-1.txz /i-data/md1/packages/xxx/
cd
rm -rf ${BUILDDIR}
ENDTIME=$(date +"%s")

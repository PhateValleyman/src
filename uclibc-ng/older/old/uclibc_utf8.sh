...
cp -a ${SCRIPTDIR}/.config.all $PWD/.config
...
IMPORTANT NOTICE: Fully compatable only with Zyxel NSA 3** series NAS'es.
Other devices with kernel different from 2.6.31.8 are only partly compatable.

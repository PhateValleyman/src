...
colormake oldconfig
# colormake uClibc-ng headers and libraries, then utils. Hardcode libraries runpath to /ffp/lib
colormake UCLIBC_EXTRA_LDFLAGS="-Wl,-rpath=/ffp/lib"
colormake utils UCLIBC_EXTRA_LDFLAGS="-Wl,-rpath=/ffp/lib"
# Install shared libs and colormake uClibc-ng-solibs package first
# Don't use install_runtime only, because libc.so will different in solibs and main package.
# colormake install does additional sed'ing for libc.so
#colormake PREFIX=${BUILDDIR} install_runtime
colormake PREFIX=${BUILDDIR} install
...
uClibc-ng (aka �Clibc-ng/pronounced yew-see-lib-see next generation) is a C library for developing
...
# Let's colormake symlinks for compatibility with older builds < 1.0.17. and uClibc
cd ${BUILDDIR}/ffp/lib
...
# Install and colormake uClibc-ng package
colormake PREFIX=${BUILDDIR} install
colormake PREFIX=${BUILDDIR} install_utils
# Include locale script (it just imitating the real locale tool) in the main uClibc-ng package
# locale is required for texinfo tests complete successfully and likely for correct texinfo tools functionality.

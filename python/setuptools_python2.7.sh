...
SOURCEURL=https://github.com/pypa/${PACKAGENAME}/archive/v${VERSION}.tar.gz
HOMEURL=https://pypi.python.org/pypi/${PACKAGENAME}
DEPNAME=certifi
DEPVERSION="2016.9.26"
DEPURL=https://pypi.io/packages/source/c/${DEPNAME}/${DEPNAME}-${DEPVERSION}.tar.gz
...
sed -i 's|/etc/ssl/cert.pem|/ffp/etc/ssl/cert.pem|' ${PACKAGENAME}/ssl_support.py
#sed -i 's|/usr/local|/ffp|g' ${PACKAGENAME}/tests/test_easy_install.py
#sed -i 's|/usr/|/ffp/|g' ${PACKAGENAME}/tests/test_easy_install.py
...
wget -nv ${DEPURL}
tar -vxf ${DEPNAME}-${DEPVERSION}.tar.gz
...
Setuptools is a fully-featured, actively-maintained, and stable library designed to easily download,
build, install, upgrade, and uninstall Python packages.
License:PSF or ZPL 
Version:${VERSION}
Homepage:${HOMEURL}

Depends on these packages:
ffp-buildtools git python-2.7.* tar
...
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/bin\/env/#!\/ffp\/bin\/env/' {} \;
# Add dependencies
echo "ffp-buildtools git python-2.7.* tar" > ${BUILDDIR}/install/DEPENDS

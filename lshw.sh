...
GNU_HOST=$GNU_BUILD
BUILDDIR=/mnt/HD_a2/build/lshw
PACKAGENAME=lshw
#SOURCEURL=https://github.com/lyonel/${PACKAGENAME}.git
SOURCEURL=/i-data/md1/COMPILER/finish/lshw
HOMEURL=http://www.ezix.org/project/wiki/HardwareLiSter
...
cd ${BUILDDIR}
git clone ${SOURCEURL}
#tar -vxf ${SOURCEURL}/${PACKAGENAME}.tar.gz
#mv ${PACKAGENAME}-master ${PACKAGENAME}
cd ${PACKAGENAME}
VERSION="cat .VERSION"
...
colormake install DESTDIR=${BUILDDIR} SBINDIR=/ffp/bin
mkdir -p ${BUILDDIR}/install
#|--------------------------------------Handy Ruller---------------------------------------------|
cat > ${BUILDDIR}/install/DESCR << EOF

Description of $PACKAGENAME:
lshw (Hardware Lister) is a small tool to provide detailed information on the hardware configuration
of the machine.
License:GPL v2
Version:${VERSION}
Homepage:${HOMEURL}
...
None. This is a static build of lshw
...
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/bin\/env/#!\/ffp\/bin\/env/' {} \;
# Copy static binary
if [ -f ${BUILDDIR}/${PACKAGENAME}/src/lshw-static ]
then
   cp -a ${BUILDDIR}/${PACKAGENAME}/src/lshw-static ${BUILDDIR}/ffp/bin/lshw
fi

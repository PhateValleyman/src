...
VERSION=0.10.1
SOURCEURL=http://libass.googlecode.com/files/libass-${VERSION}.tar.gz
HOMEURL=http://code.google.com/p/libass/
PACKAGENAME=libass
if [ ! -d ${TMPDIR} ]; then
   mkdir -p ${TMPDIR}
fi
...
echo -e "libass is a portable subtitle renderer for the ASS/SSA (Advanced Substation Alpha/Substation Alpha) subtitle format.\nIt is mostly compatible with VSFilter." >> ${SOURCEDIR}/install/DESCR
echo "Version:${VERSION}" >> ${SOURCEDIR}/install/DESCR
echo "Homepage:${HOMEURL}" >> ${SOURCEDIR}/install/DESCR
echo "" >> ${SOURCEDIR}/install/DESCR
echo "Requires libraries from: bzip2 expat fribidi freetype fontconfig gcc libiconv uClibc zlib" >> ${SOURCEDIR}/install/DESCR
echo "" >> ${SOURCEDIR}/install/DESCR

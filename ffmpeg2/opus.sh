...
VERSION=1.0.3
SOURCEURL=http://downloads.xiph.org/releases/opus/opus-${VERSION}.tar.gz
HOMEURL=http://www.opus-codec.org
PACKAGENAME=opus
if [ ! -d ${TMPDIR} ]; then
   mkdir -p ${TMPDIR}
fi
...
./configure --prefix=/ffp --enable-shared --disable-static --enable-fixed-point
make

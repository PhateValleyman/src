...
SOURCEDIR=/mnt/HD_a2/build/leptonica
VERSION=1.82.0
SOURCEURL=http://www.leptonica.org/source/leptonica-${VERSION}.tar.gz
HOMEURL=http://www.leptonica.org
...
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/usr\/bin\/perl/#!\/ffp\/bin\/perl/' {} \;
find . -type f -executable -exec sed -i -r 's/^#! ?\/usr\/bin\/env perl/#!\/ffp\/bin\/env perl/' {} \;
./configure --prefix=/ffp --disable-static
colormake
...
echo "Leptonica is useful software for image processing and image analysis applications." >> ${SOURCEDIR}/install/DESCR
echo "Version:${VERSION}" >> ${SOURCEDIR}/install/DESCR
echo "Homepage:${HOMEURL}" >> ${SOURCEDIR}/install/DESCR
echo "" >> ${SOURCEDIR}/install/DESCR
echo "Requires libraries from: s:giflib s:libjpeg s:libpng br2:libtiff br2:libwebp s:zlib s:uClibc-solibs s:gcc-solibs" >> ${SOURCEDIR}/install/DESCR
echo "" >> ${SOURCEDIR}/install/DESCR

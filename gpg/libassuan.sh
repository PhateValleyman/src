...
GNU_HOST=$GNU_BUILD
BUILDDIR=/mnt/HD_a2/build/libassuan
PACKAGENAME=libassuan
VERSION=2.5.5
#SOURCEURL=https://www.gnupg.org/ftp/gcrypt/${PACKAGENAME}/${PACKAGENAME}-${VERSION}.tar.bz2
SOURCEURL=http://192.168.1.20:88/MyWeb/COMPILER/finish/libassuan-2.5.5.tar.bz2
HOMEURL=https://www.gnupg.org
...
cd ${BUILDDIR}
wget -nv ${SOURCEURL}
tar -vxf ${PACKAGENAME}-${VERSION}.tar.bz2
...
Libassuan is a small library implementing the so-called Assuan protocol. This protocol is used
for IPC between most newer GnuPG components. Both, server and client side functions are provided. 
License:GPL and LGPL
Version:${VERSION}
Homepage:${HOMEURL}
Depends on these packages:
libgpg-error uClibc-solibs
...
cp /i-data/md1/packages/xxx/${PACKAGENAME}-${VERSION}-arm-1.txz /mnt/HD_a2/ffp0.7arm/packages/gpg/${PACKAGENAME}-${VERSION}-arm-1.txz

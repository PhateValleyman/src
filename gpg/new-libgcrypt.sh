...
GNU_HOST=$GNU_BUILD
BUILDDIR=/mnt/HD_a2/build/libgcrypt
...
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/bin\/perl/#!\/ffp\/bin\/perl/' {} \;
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/bin\/env/#!\/ffp\/bin\/env/' {} \;
# Create compatibility symlinks with old versions
cd ${BUILDDIR}/ffp/lib
libgcrypt_current="$(readlink -f ${BUILDDIR}/ffp/lib/libgcrypt.so)"
ln -s ${libgcrypt_current##*/} libgcrypt.so.0
ln -s ${libgcrypt_current##*/} libgcrypt.so.11
cd ${BUILDDIR}

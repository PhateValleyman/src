...
cd ${BUILDDIR}
#wget -nv ${SOURCEURL}
tar vxf /i-data/md1/hdd2/COMPILER/finish/${PACKAGENAME}-${VERSION}.tar.gz
...
# configure: don't add -Werror to build flags (patch is included upstream, so hack will be useless for 4.10 and later)
gl_cv_warn_c__Werror=no ./configure \
	--build=$GNU_BUILD \
	--host=$GNU_HOST \
	--prefix=/ffp \
	--with-sysroot=/ffp \

...
# qbittorrent has problem with boost1.57 https://github.com/qbittorrent/qBittorrent/issues/4112
#patch -p1 < ${SCRIPTDIR}/qbittorrent-boost1.57.patch
...
# LDFLAGS="-static", Build statically with uclibc-ng, which has support of accept4 syscall
# Sadly, but static build will fail due to missing static libtorrent and qt4 libs 
LIBS="-lubacktrace" ./configure \
		--build=$GNU_BUILD \
		--host=$GNU_HOST \
		--prefix=/ffp \
		--with-boost=/ffp \
...
EOF
### Optionally br2:python-2.7.* for torrent search tab, icu4c (both installed with boost as deps)

...
HOMEURL=http://www.gnu.org/software/grep/
...
cd ${BUILDDIR}
#wget -nv ${SOURCEURL}
#cp /i-data/md1/COMPILER/finish/${PACKAGENAME}-${VERSION}.tar.gz ${BUILDDIR}/
cp /i-data/43d6b8c3/hdd2/COMPILER/ffp_packages/grep/grep-3.8.tar.gz ${BUILDDIR}/
...
./configure \
	...
	--disable-perl-regexp
colormake
# grep test success, but gnulib 3 tests fail
#make check
...
#cp -a /i-data/md1/packages/xxx/${PACKAGENAME}-${VERSION}-arm-1.txz ${PKGDIR}/${PACKAGENAME}-${VERSION}-arm-1.txz

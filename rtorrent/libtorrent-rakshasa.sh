...
PACKAGENAME=libtorrent-rakshasa
VERSION=0.13.4
SOURCEURL=https://github.com/rakshasa/libtorrent/archive/${VERSION}.tar.gz
...
mv libtorrent-${VERSION} ${PACKAGENAME}-${VERSION}
...
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/bin\/perl/#!\/ffp\/bin\/perl/' {} \;
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/bin\/env/#!\/ffp\/bin\/env/' {} \;
# use option --disable-instrumentation to disable built-in gcc 4.70+ atomic operations
# or use gcc < 4.70 to compile this and all dependent packages
./configure \
	--build=$GNU_BUILD \
	--host=$GNU_HOST \
	--prefix=/ffp \
...
BitTorrent library with a focus on high performance and good code 
License: GPL
Version:${VERSION}
Homepage:${HOMEURL}
Depends on these packages:
gcc-solibs openssl uClibc-solibs zlib

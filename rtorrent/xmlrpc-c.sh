...
VERSION=1.42.00
SOURCEURL="http://svn.code.sf.net/p/${PACKAGENAME}/code/release_number/0${VERSION}/"
HOMEURL=http://xmlrpc-c.sourceforge.net/
...
cd ${BUILDDIR}
svn co -q ${SOURCEURL} ${PACKAGENAME}-${VERSION}
...
make -C tools install DESTDIR=${BUILDDIR}
mkdir -p ${BUILDDIR}/install
#|--------------------------------------Handy Ruller---------------------------------------------|
cat > ${BUILDDIR}/install/DESCR << EOF

Description of $PACKAGENAME:
A lightweight RPC library based on XML and HTTP. XML-RPC is a quick-and-easy way to make procedure
calls over the Internet. It converts the procedure call into an XML document, sends it to a remote
...
curl gcc-solibs libiconv libxml2 openssl uClibc-solibs xz-5* zlib

EOF

# if --enable-cplusplus, then there is more dependencies:
# curl gcc-solibs libiconv libxml2 ncurses openssl readline uClibc-solibs xz-5* zlib

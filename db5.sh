...
PACKAGENAME=db
VERSION="5.3.28"
SOURCEURL=http://download.oracle.com/berkeley-db/${PACKAGENAME}-${VERSION}.tar.gz
HOMEURL=https://www.oracle.com/database/berkeley-db/db.html
...
The Berkeley DB package contains programs and utilities used by many other applications for database
related functions.
License: custom-BerkeleyDB license.
http://www.oracle.com/technetwork/database/berkeleydb/downloads/licensing-098979.html
Version: ${VERSION}
Homepage: ${HOMEURL}
Depends on these packages:
gcc-solibs uClibc-solibs
...
makepkg ${PACKAGENAME}${VERSION%%.*} ${VERSION} 1
cp -a ${PKGDIR}/${PACKAGENAME}${VERSION%%.*}-${VERSION}-arm-1.txz ${PKGDIRBACKUP}
cd
rm -rf ${BUILDDIR}
ENDTIME=$(date +"%s")

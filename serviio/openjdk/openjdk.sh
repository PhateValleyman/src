...
cd ${BUILDDIR}
# Get openjdk hg root repository
wget -nv ${SOURCEURL}
tar -vxf ${SOURCEVERSION}.tar.gz
mv jdk8u-${SOURCEVERSION} ${PACKAGENAME}-${VERSION}
rm ${SOURCEVERSION}.tar.gz
cd ${PACKAGENAME}-${VERSION}
# Get subrepositories
for subrepo in corba hotspot jdk jaxws jaxp langtools nashorn; do
	wget -nv http://hg.openjdk.java.net/jdk8u/jdk8u/$subrepo/archive/${SOURCEVERSION}.tar.gz -O $subrepo-${SOURCEVERSION}.tar.gz
	tar -xf $subrepo-${SOURCEVERSION}.tar.gz
	mv $subrepo-${SOURCEVERSION} $subrepo
	rm $subrepo-${SOURCEVERSION}.tar.gz
done
# Apply patches
for patches in ${SCRIPTDIR}/${PACKAGENAME}-*.patch; do
	patch -p0 < $patches
done
for patches in ${SCRIPTDIR}/${PACKAGENAME}8-*.patch; do
	patch -p1 < $patches
done
...
LD_LIBRARY_PATH="/lib:/usr/lib:/usr/local/lib:/usr/local/zy-pkgs/lib:/ffp/lib" sh ./configure \
	--prefix=${BUILDDIR}/ffp \
	--build=$GNU_BUILD \
	--host=$GNU_HOST \
	--with-update-version="40" \
	--with-build-number="b25" \
	--with-milestone=ffp0.7arm \
	--with-sys-root=/ffp \
	--disable-headful \
	--with-jvm-variants=zero \
	--with-boot-jdk=/ffp/opt/java/java-7-openjdk \
	...
	--with-giflib=system \
	--with-zlib=system \
	--with-cacerts-file=/ffp/etc/ssl/certs/cacert.pem \
	--enable-unlimited-crypto \
	--with-stdc++lib=dynamic \
	--with-memory-size=192 \
	--with-extra-ldflags="-L/ffp/lib -Wl,-rpath,/ffp/lib" \
	--with-extra-cflags="-I/ffp/include -D__STDC_LIMIT_MACROS -Wno-unused-parameter" \
	--with-extra-cxxflags="-I/ffp/include -D__STDC_LIMIT_MACROS -Wno-unused-parameter"
LD_LIBRARY_PATH="/lib:/usr/lib:/usr/local/lib:/usr/local/zy-pkgs/lib:/ffp/lib" make DEBUG_BINARIES=true images

...
VERSION="1.1.0"
SOURCEURL=ftp://ftp.alsa-project.org/pub/lib/${PACKAGENAME}-${VERSION}.tar.bz2
HOMEURL=http://www.alsa-project.org
...
wget -e robots=off -nv -np -nd -r --level=1 -H -A patch http://git.buildroot.net/buildroot/plain/package/alsa-lib/
for patches in ????-*.patch; do
    patch -p1 < "$patches"
done
# Change path to asound.conf
sed -i 's|/etc/asound.conf|/ffp/etc/asound.conf|g' src/conf/cards/USB-Audio.conf src/conf/alsa.conf src/conf.c
...
The Advanced Linux Sound Architecture (ALSA) provides audio and MIDI functionality to the Linux
operating system. The ALSA Library package contains the ALSA library used by programs (including
ALSA Utilities) requiring access to the ALSA sound interface.
License:GNU GPL and LGPL
Version:${VERSION}
Homepage:${HOMEURL}

Depends on these packages:
br2:python-2.7.* br2:uClibc-solibs

...
VERSION="1.4.11"
SOURCEURL=http://mosquitto.org/files/source/${PACKAGENAME}-${VERSION}.tar.gz
HOMEURL=https://mosquitto.org/
...
sed -i 's|/etc/mosquitto|/ffp/etc/mosquitto|g' Makefile
sed -i 's|/sbin|/bin|g' src/Makefile
sed -i 's| -lanl||' config.mk
# Set default service run as system user to nobody (either ways test fails. Another option to creater sys user-mosquitto)
sed -i 's|config->user = "mosquitto";|config->user = "nobody";|' src/conf.c
make \
	prefix=/ffp \
	WITH_WEBSOCKETS=yes
make test
make prefix=/ffp DESTDIR=${BUILDDIR} install
mkdir -p ${BUILDDIR}/install
#|--------------------------------------Handy Ruller---------------------------------------------|
cat > ${BUILDDIR}/install/DESCR << EOF

Description of $PACKAGENAME:
Eclipse Mosquitto is an open source message broker that implements the MQTT protocol versions 3.1 and 3.1.1.
MQTT provides a lightweight method of carrying out messaging using a publish/subscribe model.
This makes it suitable for "Internet of Things" messaging such as with low power sensors or mobile devices
such as phones, embedded computers or microcontrollers like the Arduino.
License(s): EPLv1.0 or EDLv1.0
Version: ${VERSION}
Homepage: ${HOMEURL}
...
# Move default config
mv "${BUILDDIR}/ffp/etc/${PACKAGENAME}/${PACKAGENAME}.conf.example" "${BUILDDIR}/ffp/etc/${PACKAGENAME}/${PACKAGENAME}.conf"
# Add c-ares and libwebsockets to list of deps (c-ares and libwebsockets packages are not uploaded to repo yet)
sed -i "s|$| c-ares libwebsockets|" ${BUILDDIR}/install/DEPENDS

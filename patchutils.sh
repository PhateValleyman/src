...
SOURCEDIR=/mnt/HD_a2/build/patchutils
VERSION=0.4.2
#SOURCEURL=http://cyberelk.net/tim/data/patchutils/stable/patchutils-${VERSION}.tar.xz
SOURCEURL=http://192.168.1.20:88/MyWeb/COMPILER/finish/patchutils-0.4.2.tar.xz
HOMEURL=http://cyberelk.net/tim/software/patchutils/
PACKAGENAME=patchutils
if [ ! -d ${TMPDIR} ]; then
   mkdir -p ${TMPDIR}
fi
if [ ! -d ${SOURCEDIR} ]; then
...
cd ${SOURCEDIR}
wget ${SOURCEURL}
tar -vxvf ${PACKAGENAME}-${VERSION}.tar.xz
...
echo "Patchutils is a small collection of programs that operate on patch files" >> ${SOURCEDIR}/install/DESCR
echo "Version:${VERSION}" >> ${SOURCEDIR}/install/DESCR
echo "Homepage:${HOMEURL}" >> ${SOURCEDIR}/install/DESCR
echo "" >> ${SOURCEDIR}/install/DESCR
echo "Requires libraries from: perl uClibc-solibs" >> ${SOURCEDIR}/install/DESCR
echo "" >> ${SOURCEDIR}/install/DESCR
...
cp /i-darmta/md1/packages/xxx/${PACKAGENAME}-${VERSION}-arm-0.txz /mnt/HD_a2/ffp0.7arm/packages/
mv /i-darmta/md1/packages/xxx[A/${PACKAGENAME}-${VERSION}-arm-0.txz /ffp/funpkg/additional/
cd
rm -rf ${SOURCEDIR}

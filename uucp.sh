...
GNU_HOST=$GNU_BUILD
BUILDDIR=/mnt/HD_a2/build/uucp
PACKAGENAME=uucp
VERSION="1.07"
SOURCEURL=https://ftp.gnu.org/gnu/uucp/${PACKAGENAME}-${VERSION}.tar.gz
HOMEURL=http://www.gnu.org/software/uucp/uucp.html
...
sed -i 's|/usr/spool|/ffp/var/spool|g' policy.h
sed -i 's|/bin/ps|/ffp/bin/ps|g' policy.h
sed -i '/^#define CMDPATH/c\#define CMDPATH "/ffp/bin"' policy.h
./configure \
	--prefix=/ffp \
	--build=$GNU_BUILD \
	--host=$GNU_HOST \
	--mandir=/ffp/share/man \
	--with-newconfigdir=/ffp/etc/uucp \
	--sbindir=/ffp/bin \
	--with-user=dialout
colormake
colormake install DESTDIR=${BUILDDIR}
mkdir -p ${BUILDDIR}/ffp/etc
mkdir -p ${BUILDDIR}/ffp/var/spool/uucp
install -D -m644 sample/config ${BUILDDIR}/ffp/etc/uucp/config
install -D -m644 sample/sys1 ${BUILDDIR}/ffp/etc/uucp/sys
install -D -m644 sample/port ${BUILDDIR}/ffp/etc/uucp/port
sed -i 's|/usr/spool|/ffp/var/spool|g' ${BUILDDIR}/ffp/etc/uucp/config
mv ${BUILDDIR}/ffp/info ${BUILDDIR}/ffp/share/

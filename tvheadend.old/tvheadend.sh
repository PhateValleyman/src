...
# Workaround of problem for gcc 4.92 and above is below
set -e
...
bzip2 dbus dvb-apps ffmpeg3/ libiconv openssl tar uClibc-solibs uriparser v4l-dvb v4l-utils zlib
eudev kmod expat

EOF
echo ${HOMEURL} > ${BUILDDIR}/install/HOMEPAGE
# Copy postinstall script to remove old start-up script
# and create preconfigs for admin and dvr
CHECKSUM=$(md5sum ${BUILDDIR}/ffp/start/${PACKAGENAME}.sh|awk '{print $1}')
cp -a ${SCRIPTDIR}/tvheadenddoinst.sh ${BUILDDIR}/install/doinst.sh
sed -i "s|\<CHECKSUM\>|$CHECKSUM|" ${BUILDDIR}/install/doinst.sh

...
set -e
# Suppress warning: typedef locally defined but not used
export CPPFLAGS="$CPPFLAGS -Wno-unused-local-typedefs"
export SHELL="/ffp/bin/sh"
GNU_BUILD=arm-ffp-linux-uclibceabi
GNU_HOST=$GNU_BUILD
BUILDDIR=/mnt/HD_a2/build/pkg-config
PACKAGENAME=pkg-config
VERSION="0.29.2"
SOURCEURL=http://pkgconfig.freedesktop.org/releases/${PACKAGENAME}-${VERSION}.tar.gz
HOMEURL=http://pkgconfig.freedesktop.org/wiki/
STARTTIME=$(date +"%s")
...
# Correct prefix to successfully pass tests
find check -type f -iname "*.pc" -exec sed -i 's|prefix=/usr|prefix=/ffp|g' {} \;
find check -type f -executable -exec sed -i 's|/usr|/ffp|g' {} \;
...
./configure \
	...
	--prefix=/ffp \
	--with-internal-glib \
	--with-system-include-path=/ffp/include \
	--with-system-library-path=/ffp/lib
#	--with-pc-path
...
The pkg-config package contains a tool for passing the include path and/or library paths to build
tools during the configure and make file execution. It helps to insert the correct compiler options
on the command line. 
License:GNU GPLv2+
Version:${VERSION}
Homepage:${HOMEURL}
...
br2:libiconv br2:uClibc br2:uClibc-solibs

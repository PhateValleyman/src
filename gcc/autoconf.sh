...
GNU_HOST=$GNU_BUILD
BUILDDIR=/mnt/HD_a2/build/autoconf
PACKAGENAME=autoconf
VERSION="2.71"
#SOURCEURL=https://ftp.gnu.org/gnu/${PACKAGENAME}/${PACKAGENAME}-${VERSION}.tar.gz
SOURCEURL=http://192.168.1.20:88/MyWeb/COMPILER/finish/autoconf-2.71.tar.gz
HOMEURL=http://www.gnu.org/software/autoconf/autoconf.html
STARTTIME=$(date +"%s")
...
License:GNU GPLv2, GPLv3
Version:${VERSION}
Homepage:${HOMEURL}

Depends on these packages:
br2:m4 s:perl br2:perl-modules s:gawk s:diffutils
...
find . -type f -iname "*.pl*" -exec sed -i -r 's/^#! ?\/bin\/env/#!\/ffp\/bin\/env/' {} \;
# Correct hardcoded shell
sed -i 's|CONFIG_SHELL-/bin/sh|CONFIG_SHELL-/ffp/bin/sh|' ${BUILDDIR}/ffp/bin/autoconf
# Add dependencies manually
cat >> ${BUILDDIR}/install/DEPENDS <<- EOF
m4 perl perl-modules gawk diffutils
EOF

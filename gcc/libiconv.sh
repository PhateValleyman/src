...
# Update with some patches from git master branch
for p in ${SCRIPTDIR}/${PACKAGENAME}*.patch; do
	cp -a $p $PWD/
	patch -p1 < $p
done
# Correct hardcoded shell
find . -type f -iname "Makefile*" -exec sed -i -r 's|/bin/sh|/ffp/bin/sh|g' {} \;
...
Libiconv is a character set conversion library.
...
br2:uClibc br2:uClibc-solibs br2:gettext
...
cp /tmp/${PACKAGENAME}-${VERSION}-arm-2.txz /mnt/HD_a2/ffp0.7arm/packages/gcc/
mv /tmp/${PACKAGENAME}-${VERSION}-arm-2.txz /ffp/funpkg/additional/gcc/
cd
rm -rf ${BUILDDIR}
ENDTIME=$(date +"%s")

...
VERSION="1.1.1"
SOURCEURL=http://downloads.xiph.org/releases/theora/${PACKAGENAME}-${VERSION}.tar.xz
HOMEURL=http://www.xiph.org/theora/
...
# Apply patch for compatibility with libpng 1.6.* version 
patch -p0 < "${SCRIPTDIR}/${PACKAGENAME}"-????\.patch
...
# Remove empty dir
if	[ "$(find ${BUILDDIR}/ffp/share -prune -empty)" ]
then
	rm -rf ${BUILDDIR}/ffp/share
fi

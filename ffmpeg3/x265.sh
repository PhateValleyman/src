...
PACKAGENAME=x265
VERSION="2.4"
SOURCEURL=https://bitbucket.org/multicoreware/${PACKAGENAME}/downloads/${PACKAGENAME}_${VERSION}.tar.gz
HOMEURL=http://x265.org/
...
cd ${BUILDDIR}
wget -nv ${SOURCEURL}
tar xf "${SOURCEURL##*/}" --transform=s"|${PACKAGENAME}_${VERSION}|${PACKAGENAME}-${VERSION}|"
cd ${PACKAGENAME}-${VERSION}
# Apply patch for armv5 arch and enable static lib build
# Source1: https://raw.githubusercontent.com/archlinuxarm/PKGBUILDs/master/extra/x265/arm.patch
# Source2: http://www.linuxfromscratch.org/patches/blfs/svn/x265_2.4-enable_static-1.patch
for patches in ${SCRIPTDIR}/${PACKAGENAME}-????\.patch
do
    patch -p1 < "$patches"
done

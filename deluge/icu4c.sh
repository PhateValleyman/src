...
### REMEMBER #### Before compile uninstall old ICU package, otherwise it course compilation error.
#export CPPFLAGS="-I/ffp/include -DU_USING_ICU_NAMESPACE=0"
...
PACKAGENAME=icu4c
VERSION="58.2"
SOURCEURL=http://download.icu-project.org/files/${PACKAGENAME}/${VERSION}/${PACKAGENAME}-${VERSION//./_}-src.tgz
HOMEURL=http://site.icu-project.org/
...
wget -Lnv ${SOURCEURL}
tar xf "${SOURCEURL##*/}"
mv icu ${PACKAGENAME}-${VERSION}
cd ${PACKAGENAME}-${VERSION}
# Apply patches
wget -e robots=off -nv -np -nd -H -r --level=1 -A patch https://git.buildroot.net/buildroot/plain/package/icu/
for patches in *.patch; do
    patch -p1 < $patches
done
wget -nv 'https://git.archlinux.org/svntogit/packages.git/plain/trunk/icu-58.1-iterator-reset.patch?h=packages/icu' -O 1.patch
patch -p3 < 1.patch
...
The International Components for Unicode (ICU) package is a mature, widely used set of C/C++ libraries,
providing Unicode and Globalization support for software applications. ICU is widely portable and gives
applications the same results on all platforms.
License: custom icu open source (http://www.unicode.org/copyright.html#License)
Version: ${VERSION}
Homepage: ${HOMEURL}

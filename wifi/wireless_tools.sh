...
VERSION=30
PREVERSION=pre9
SOURCEURL=http://www.hpl.hp.com/personal/Jean_Tourrilhes/Linux/${PACKAGENAME}.${VERSION}.${PREVERSION}.tar.gz
HOMEURL=http://www.labs.hpe.com/personal/Jean_Tourrilhes/Linux/Tools.html
...
make CFLAGS="${CPPFLAGS} ${CFLAGS} -I." LDFLAGS="${LDFLAGS}"
make \
	INSTALL_DIR="${BUILDDIR}/ffp/bin" \
	INSTALL_LIB="${BUILDDIR}/ffp/lib" \
	INSTALL_INC="${BUILDDIR}/ffp/include" \
	INSTALL_MAN="${BUILDDIR}/ffp/share/man" \
	install
...
makepkg ${PACKAGENAME} ${VERSION}.${PREVERSION} 1
cp -a ${PKGDIR}/${PACKAGENAME}-${VERSION}.${PREVERSION}-arm-1.txz ${PKGDIRBACKUP}
cd
rm -rf ${BUILDDIR}
ENDTIME=$(date +"%s")
